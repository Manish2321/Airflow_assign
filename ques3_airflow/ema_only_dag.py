...
with DAG(
    ...
    fetch_Aus_data = PythonOperator(
        ...
        op_args=['Australia', '/Users/manishkumartyagi/airflow_snowflake/output/usa_output.csv','data/retail_sales/aus_output.csv'],
    )

    fetch_Belgium_data = PythonOperator(
        task_id='fetch_belgium_data',
        python_callable=fetch_and_write_to_csv,
        op_args=['Belgium', '/Users/manishkumartyagi/airflow_snowflake/output/india_output.csv','data/retail_sales/belgium_output.csv'],
    )

    fetch_Brazil_data = PythonOperator(
        task_id='fetch_brazil_data',
        python_callable=fetch_and_write_to_csv,
        op_args=['Brazil', '/Users/manishkumartyagi/airflow_snowflake/output/canada_output.csv','data/retail_sales/brazil_output.csv'],
    )
